  ...
  /* ── Calls ── */
  {
    ...
    endpoints: [
      ...
      },
      {
        ...
        description: 'Update call status. Setting status to "active" records answered_at. Setting "ended" records ended_at and calculates duration.',
        ...
        responses: [
          { status: 200, description: 'Updated call log' },
    ...
    events: [
      {
        ...
        payload: { token: 'JWT access token' },
        response: 'authenticated → {user, online_users[]}',
      },
      {
        name: 'get_online_users', direction: 'emit',
        summary: 'Request current online users',
        response: 'online_users → {users[]}',
      },
      {
        name: 'heartbeat', direction: 'emit',
        summary: 'Keep-alive heartbeat',
        response: 'heartbeat_ack → {status, user_id}',
      },
      {
        name: 'user_status_changed', direction: 'listen',
        summary: 'User came online/offline',
        payload: { user_id: 'integer', status: '"online" | "offline"', username: 'string' },
      },
    ],
  },
  {
